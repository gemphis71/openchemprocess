...
本文件对投料顺序的所有讨论，均**隐含建立在“有效混合态已成立”**的前提之上。

即：在投料发生的时间尺度内，加入的物料能够进入反应主体体积，并参与整体混合过程；  
而非发生局部滞留、沉积、脱耦或进入无效体积（如死角、阀腔等）。

若该前提不成立，则投料顺序规则本身不再适用，  
风险模式将退化为“有效进入失败”或“局部滞留”型问题，  
应在 CHG-002（effective dosing）或后续章节中讨论，而非作为顺序风险处理。
...
---
### P4｜强触发型顺序风险（P3 的强触发型子类）

**定义**  
P4 是 P3（先蓄积、后触发）的一种强触发型子类，其触发步骤通过**底物分解或高活性中间体生成**来推进反应。

**结构特征**
- 触发物加入本身即为反应触发
- 反应推进依赖底物分解或瞬时高活性物种生成
- 反应速率、放热与放气不可连续调节
- 体系物理表现为 **violent**（爆发性、不可控）
...
- 禁止：先蓄积反应物，再加入触发物
- 必须：触发物先加入并充分分散
- 必须：反应物仅可在目标反应温度下加入，且加料速率不得超过体系可控反应速率。
